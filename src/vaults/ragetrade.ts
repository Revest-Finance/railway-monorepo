...
const APY_ENDPONT = 'https://apis.rage.trade/data/v2/get-dn-gmx-apy-breakdown?networkName=arbmain'
...
export async function rageTrade() {
    ...
    await updateVault({
        ...
        apy:  (apy_res.result.juniorVault.glpTraderPnl 
            + apy_res.result.juniorVault.glpRewardsPct 
            + apy_res.result.juniorVault.esGmxRewards
            + apy_res.result.juniorVault.btcBorrowApy
            + apy_res.result.juniorVault.ethBorrowApy) / 100,
        tvl: parseFloat(ethers.formatUnits(
